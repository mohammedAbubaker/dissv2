...
__global__ void ray_aabb_intersect_top16(
    ...
    int* out_indices,        // [R, 16] output indices
    ...
    // Arrays to hold top 16 intersections
    float top_t[16];
    int top_idx[16];
    ...
    for (int box_idx = 0; box_idx < num_boxes; box_idx++) {
        ...
        if (tmax >= tmin && tmax > 0) {
            ...
            // Check if this hit is among the 16 closest
            if (t_hit < top_t[15]) {
    ...
    for (int i = 0; i < 16; i++) {
        out_indices[ray_idx * 16 + i] = top_idx[i];
    }
}
"""